...
:host_("localhost"), port_("31000"), socket_(io_service_),running_(false)
{}

OTBioLabClient::OTBioLabClient(const string& host)
:host_(host), port_("31000"), socket_(io_service_), running_(false)
{
  try
  {
...
void OTBioLabClient::connect(const string& host)
{
  try
  {
    tcp::resolver resolver(io_service_);
